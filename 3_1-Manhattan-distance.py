...
# function that calculates new path for the wire and returnes coordinates as a set to be added to the existing set along with the new cursor coordinates so that the next step/set can be "plotted"
def drawCoordinates(commandInput, currentPoint):
	# separate commandInput into direction and steps ( first letter is a command for direction then the number of steps) 
	direction = commandInput[0]
	steps = int(commandInput[1:])
	newCoordinates = set()
	if direction.lower() == 'r':
		for i in range(steps):
			newCoordinates.add((currentPoint[0] + i, currentPoint[1]))
		currentPoint[0] = int(currentPoint[0]) + steps
		...
		for i in range(steps):
			newCoordinates.add((currentPoint[0], currentPoint[1] + i))
		currentPoint[1] = int(currentPoint[1]) + steps
	elif direction.lower() == 'd':
		for i in range(steps):
			newCoordinates.add((currentPoint[0], currentPoint[1] - i))
		currentPoint[1] = int(currentPoint[1]) - steps
	return newCoordinates, currentPoint


firstWire = firstInput.split(',')
secondWire = secondInput.split(',')
currentPoint = [0,0]
firstWireCoordinates = set()
secondWireCoordinates = set()
...
for i in range(len(firstWire)):
	newCoordinates, currentPoint = drawCoordinates(firstWire[i], currentPoint)
...
for i in range(len(secondWire)):
	newCoordinates, currentPoint = drawCoordinates(secondWire[i], currentPoint)
	secondWireCoordinates.update(newCoordinates)

# find intersection of two lists, remove (0,0) and calculate Manhattan distance
intersectionOfWires = list(firstWireCoordinates & secondWireCoordinates)
for i in range(len(intersectionOfWires)):
	if abs(intersectionOfWires[i][0]) + abs(intersectionOfWires[i][1]) == 0:
		intersectionOfWires.remove(intersectionOfWires[i])
		break

for i in range(len(intersectionOfWires)):
	intersectionOfWires[i] = abs(intersectionOfWires[i][0]) + abs(intersectionOfWires[i][1])

print("Manhattan distance:", min(intersectionOfWires))
