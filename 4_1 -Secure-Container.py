...
for password in range(passwordStarts, (passwordEnds +1)):
	...
	if ((passwordString[0] <= passwordString[1]) and (passwordString[1] <= passwordString[2]) and (passwordString[2] <= passwordString[3]) and 
		...
		if (any(2 in sublist for sublist in countNumbersList) or any(3 in sublist for sublist in countNumbersList) or any(4 in sublist for sublist in countNumbersList) or
			any(5 in sublist for sublist in countNumbersList) or any(6 in sublist for sublist in countNumbersList)):
			numberOfHits += 1
